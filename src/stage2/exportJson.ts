...
function makeEvent(
  base: Pick<
    Stage2Event,
    "command_index" | "raw_command" | "clock_text" | "elapsed_sec" | "video_timestamp_sec"
  >,
...
function normalizeResult(
  tok: string,
): { result: Stage2Result | null; warning: string | null } {
  switch (tok) {
    case "make":
    case "made":
    case "score":
...
function formatElapsedLocal(tMs: number | null): string {
  if (tMs === null) return "--:--";
